...
const seed2020 = "373373373373373"; //A*3+B*7+C*3+D*3+E*7+F*3+G*3+H*7+I*3+J*3+K*7+L*3+M*3+N*7+O*3
...
module.exports = {
  getAccountBanks: (req, res, next) => {
    ...
    getAccountBanks2020: (req, res, next) => {
    let accountNumber = req.params.account;

      let accountBanks = [];

      banks.forEach((item) => {
        if (isBankAccountValid2020(accountNumber, item.code)) {
          accountBanks.push(item);
        }
      });
...
//https://www.cbn.gov.ng/out/2020/psmd/revised%20standards%20on%20nigeria%20uniform%20bank%20account%20number%20(nuban)%20for%20banks%20and%20other%20financial%20institutions%20.pdf
const generateCheckDigit2020 = (serialNumber, bankCode) => {
  if (serialNumber.length > serialNumLength) {
    throw new Error(
      `Serial number should be at most ${serialNumLength}-digits long.`
    );
    ...
    serialNumber = serialNumber.padStart(serialNumLength, "0");
    bankCode = bankCode.padStart(6, "0");
    let cipher = bankCode + serialNumber;
    //cipher must be 15 digit long
    console.log("cipher", cipher);
  let sum = 0;

  // Step 1. Calculate A*3+B*7+C*3+D*3+E*7+F*3+G*3+H*7+I*3+J*3+K*7+L*3
  cipher.split("").forEach((item, index) => {
    sum += item * seed2020[index];
  });
...
const isBankAccountValid2020 = (accountNumber, bankCode) => {
  if (!accountNumber || !accountNumber.length == nubanLength) {
    error = "NUBAN must be %s digits long" % nubanLength;
    return false;
  }
  ...
  let checkDigit = generateCheckDigit2020(serialNumber, bankCode);
